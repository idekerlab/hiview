...
class Highlight extends AbstractSeries {
  ...
  _getDrawArea(xLoc, yLoc) {
    ...
    const rightBorder =
      xLoc < startLocX ? startLocX : Math.min(xLoc, innerWidth - marginLeft)

    const topBorder = yLoc < startLocY ? Math.max(yLoc, marginTop) : startLocY
    const bottomBorder =
      yLoc < startLocY ? startLocY : Math.min(yLoc, innerHeight - marginTop)
  ...
  _getDragArea(xLoc, yLoc) {
    ...
    const rightBorder = Math.max(
      marginLeft,
      Math.min(xLoc, innerWidth - marginLeft)
    )

    const topBorder = startLocY
    const bottomBorder = Math.max(
      marginTop,
      Math.min(yLoc, innerHeight - marginTop)
    )
